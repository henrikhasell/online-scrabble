...
from .game import Game, GameError, Placement, Player, ScoredPlacement, SolutionBuilder, Trie
...
model_grid = api.model('Grid', {
    'tiles': fields.List(
        fields.Nested(model_tile, required=True),
        required=True
    ),
...
model_message = api.model('Message', {
    'message': fields.String(required=True)
})

model_player_state = api.model('PlayerState', {
    'rack': fields.List(
        fields.String(example="A", required=True),
        required=True
    ),
    'score': fields.Integer(required=True)
})

model_game_and_player_state = api.model('GameAndPlayerState', {
...
class ActionJoinResource(Resource):
    @api_login_required(api)
    @action_namespace.doc(responses={
...
class ActionScorePlacement(Resource):
    ...
    def put(self, id: str):
        placement = Placement.from_json(api.payload)
        return marshal(
            score_placement(id, placement).json(),
